...
dict_opcodes = {"add":"00000", "sub":"00001", "mov": ["00010", "00011"], "ld":"00100", "st":"00101", 
                ...
                "subf":"10001", "movf":"10010", "incf":"10100","decf":"10101","rrl":"10110",
                "rrr":"10111","swap":"11000"}
# Types of instructions :- 
# Type A: 3 register type - add, sub, mul, xor, or, and, addf, subf
# Type B: register and immediate type - mov[0], rs, ls, incf, decf, rrl, rrr
# Type C: 2 registers type - mov[1], div, not, cmp, swap
...
list_var = []
dict_var = {}
list_typeA = ["add","sub","mul","xor","or","and","addf","subf"]
list_typeB = ["mov","rs","ls","incf","decf","rrl","rrr"]
list_typeC = ["mov","div","not","cmp","swap"]
list_typeD = ["ld","st"]
...
# Illegal Imm value (floating point)
for item in list_of_assembly_inst2:
    if item[0]=="movf":
        floatvalue=float(item[-1][1:])
        if floatvalue>=0 and floatvalue<=127:
            strans = floattobin(floatvalue)
            ...
            break
if (errorimmvaluef):
    newstring+=f"Error in line no. {lineno} - Illegal Imm value - Number can't be represented in 8 bit system (out of range)"
    print(newstring) 
    exit()
...
for lines in lineinput:
    ...
    for i in line:
        ...
        if i[0] == "$" and line[0]=="movf":
                strans = floattobin(float(i[1:]))
                single_line += check8bitsfloat(strans)

        if i[0] == '$' and line[0]!="movf":
                single_line += dec_bin(i[1:])
